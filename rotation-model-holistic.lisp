...
(defparameter *human-data* '(1.65 2.10 2.30 2.85 3.05 4.20 3.85 3.85 5.20 4.75))
...
(define-model holistic-rotation
  ...
  (chunk-type rotation-goal state xloc pattern-count (previous-color t))
  ...
  (define-chunks (true isa chunk) (false isa chunk) (square isa chunk) (rotate isa chunk)
    ...
    (encode-rotated-pattern isa chunk) (process-rotated-pattern isa chunk) (pattern-count isa chunk)
    (encode-normal-pattern isa chunk) (attend-normal-pattern isa chunk) (patterns-the-same? isa chunk))
...
(p same-normal-pattern-threashold-not-reached
    =goal>
     state attend-normal-pattern
   ...
   ==>
    !eval! (format t "Same colour: ~A Pattern count: ~A~%" =col =new-pcount)
    +visual-location>
     isa visual-location
     > screen-x 300
     - color compound
...
(p same-normal-pattern-threshold-reached
    =goal>
     state attend-normal-pattern
     pattern-count =pcount
     pattern-count 1
